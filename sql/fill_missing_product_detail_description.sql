-- SQL BÙ MÔ TẢ CHI TIẾT CHO CÁC SẢN PHẨM CÒN THIẾU
-- Dựa trên dữ liệu gốc của bảng hanghoa (sqlhanghoa.txt)
...
-- 2) Chỉ thêm các MaHang còn thiếu mô tả
...
WHERE m.MaHang IS NULL;

-- 3) (Khuyến nghị) tạo/cập nhật view detail cho trang khách xem full thông tin
...
-- 4) Query kiểm tra nhanh còn thiếu mô tả không (kỳ vọng = 0)
-- SELECT COUNT(*) AS so_san_pham_chua_co_mota
-- FROM hanghoa h
-- LEFT JOIN hanghoa_mota_chitiet m ON m.MaHang = h.MaHang
-- WHERE m.MaHang IS NULL;
